...
alphabetically, Z as A. And so, to say HELLO to someone, Caesar might write IFMMP instead. Upon receiving such messages from
Caesar, recipients would have to “decrypt” them by shifting letters in the opposite direction by the same number of places.
...
Unencrypted text is generally called plaintext. Encrypted text is generally called ciphertext. And the secret used is
called a key.
...
More formally, Caesar’s algorithm (i.e., cipher) encrypts messages by “rotating” each letter by 'k' positions. More formally, 
if 'p' is some plaintext (i.e., an unencrypted message), p(i) is the 'i'th character in 'p' , and 'k' is a secret key
(i.e., a non-negative integer), then each letter, c(i), in the ciphertext, 'c' is computed as
...
but it’s really just a concise way of expressing the algorithm precisely. Indeed, for the sake of discussion, think of A (or a)
as 0 , B (or b) as 1, …, H (or h) 7, I (or i) as 8, …, and Z (or z) as 25. Suppose that Caesar just wants to say Hi to someone
confidentially using, this time, a key, 'k', of 3. And so his plaintext 'p', is Hi, in which case his plaintext’s first character,
'p(0)' , is H (aka 7), and his plaintext’s second character, 'p(1)', is i (aka 8). His ciphertext’s first character, 'c(0)' , is
 thus K, and his ciphertext’s second character, 'c(1)', is thus L. Make sense?
